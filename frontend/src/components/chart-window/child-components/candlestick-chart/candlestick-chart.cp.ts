...
export function useDateFunctions(
  ...
  function addDateToDatePositionEntries(
    ...
    if (timeDisplayProps.value.period === TimeModePeriod.Minute) {
      ...
      if (timeDisplayProps.value.period === TimeModePeriod.Day) {
        ...
        if (timeDisplayProps.value.mode === TimeMode.Y2) {
          if (timeFrame.value.includes('D')) {
            if (monthTransition && month % 2 === 0) {
              showEntryDateFormat = 'MMM';
            }
          } else if (
            !timeFrame.value.includes('W') &&
            month === 6 &&
            monthTransition
          ) {
            showEntryDateFormat = 'MMM';
            ...
            monthTransition &&
            month === 6 &&
            !['W3', 'W4'].includes(timeFrame.value)
          ) {
            showEntryDateFormat = 'MMM';
          }
        }
  ...
  const standardDateFormat = computed(() => {
    ...
    if (timeFrameModeChar.value === 'D' || timeFrameModeChar.value === 'W') {
