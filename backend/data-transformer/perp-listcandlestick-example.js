...
d1.data.listCandleSticks.items.forEach(item => {
  res.push({high: item.high, low: item.low, close: item.close, volume: Number(item.volume), timestamp: item.startTime * 1000});
})
// d2 etc. (you have to name the single requests with list candlesticks items each)

const headers = Object.keys(res[0]).join(',');
const rows = res.filter(obj => obj.volume > 0).map(obj => Object.values(obj).join(','));
